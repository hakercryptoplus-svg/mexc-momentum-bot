...
class MEXC:
    ...
    def get_precision(self, symbol):
        """Get quantity/price precision for a symbol"""
        s = symbol.replace('/', '')
        r = self._get("/api/v3/exchangeInfo", {'symbol': s})
        if 'error' in r: return 4, 4
        try:
            sym = r['symbols'][0]
            qty_prec = int(sym['baseAssetPrecision'])
            price_prec = int(sym['quotePrecision'])
            return qty_prec, price_prec
        except:
            return 4, 4

    def get_symbol_filters(self, symbol):
        """Get LOT_SIZE filter for min/max/step qty"""
        s = symbol.replace('/', '')
        r = self._get("/api/v3/exchangeInfo", {'symbol': s})
    ...
    def check_symbol(self, symbol):
        """Verify symbol exists and is tradable on MEXC"""
        s = symbol.replace('/', '')
        r = self._get("/api/v3/exchangeInfo", {'symbol': s})
